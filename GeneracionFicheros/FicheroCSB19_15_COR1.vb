...
#Region " Constantes Modelo CSB19_15 "

Public Module CodigoRegistroCSB19_15
...
Public Module NumeroDatoCSB19_15
...
Public Module TipoAdeudoCSB19_15
...
Public Module PrefijoFichAdeudoCSB19_15
...
Public Class FicheroCSB19_15_COR1
    ...
    <Task()> Public Shared Function GenerarFichero(ByVal data As DataGenerarFichero, ByVal services As ServiceProvider) As List(Of String)
        ...
        ProcessServer.ExecuteTask(Of DataAddRegistroFichero)(AddressOf FichCSB19_15_RegistroCabeceraGeneral, datFichero, services)
        ProcessServer.ExecuteTask(Of DataAddRegistroFichero)(AddressOf FichCSB19_15_RegistrosAcreedores, datFichero, services)
        ProcessServer.ExecuteTask(Of DataAddRegistroFichero)(AddressOf FichCSB19_15_RegistroTotalesGeneral, datFichero, services)
...
#Region " Identificación del Fichero "
    ...
    <Task()> Public Shared Function GetIdentificacionFichero(ByVal data As Object, ByVal services As ServiceProvider) As String
        ...
        Return PrefijoFichAdeudoCSB19_15.FICHERO_PRESEN_ADEUDOS & Format(Today, FormatosFechas.AAAAMMDD) & Format(Date.Now, FormatosFechas.HHmmssfffff) & datOrdenante.NIF
    End Function
...
#Region " Cabecera y Totales "
    ...
    <Task()> Public Shared Sub FichCSB19_15_RegistroCabeceraGeneral(ByVal data As DataAddRegistroFichero, ByVal services As ServiceProvider)
        ...
        lstRegistro.Add(New DataCampoRegistro("Código de Registro", String.Empty, True, enumTipoCampo.Numerico, 2, CodigoRegistroCSB19_15.REG_CABECERA_GENERAL))
        lstRegistro.Add(New DataCampoRegistro("Versión Cuaderno", String.Empty, True, enumTipoCampo.Numerico, 5, CodigoCuadernoSEPA.CSB19_14_REDUCIDO))
        lstRegistro.Add(New DataCampoRegistro("Número de Dato", String.Empty, True, enumTipoCampo.Numerico, 3, NumeroDatoCSB19_15.CABECERA))
    ...
    <Task()> Public Shared Sub FichCSB19_15_RegistroTotalesGeneral(ByVal data As DataAddRegistroFichero, ByVal services As ServiceProvider)
        Dim TotGeneral As RegTotalesGeneral = services.GetService(Of RegTotalesGeneral)()

        Dim lstRegistro As New ListaCamposRegistro
        lstRegistro.Add(New DataCampoRegistro("Código de Registro", String.Empty, True, enumTipoCampo.Numerico, 2, CodigoRegistroCSB19_15.REG_TOTALES_GENERAL))
...
#Region " Registros de Acreedores - Cabeceras, Totales e Individuales "
    ...
    'Public Sub FichCSB19_15_RegistrosAcreedores(ByVal data As DataAddRegistroFichero, ByRef datOrdenante As DatosOrdenante, ByVal datBanco As DatosBancoPropioFicheros, ByVal ProcInfo As ProcessInfoFichero, ByRef TotGeneral As RegTotalesGeneral)
    '    Dim TotAcreedores As New RegTotalesAcreedores
    '    Dim TotIndividuales As New RegTotalesIndividuales

    '    If ProcInfo.OrigenDatos Is Nothing OrElse ProcInfo.OrigenDatos.Rows.Count = 0 Then Exit Sub

    '    'Dim htFechas As New Hashtable
    '    'Dim FechasCobro(-1) As Date

    '    'Dim dvFechasCobro As New DataView(ProcInfo.OrigenDatos, "NOT FechaVencimiento IS NULL", "FechaVencimiento", DataViewRowState.CurrentRows)
    '    'If dvFechasCobro.Count > 0 Then
    '    '    For Each drv As DataRowView In dvFechasCobro
    '    '        If Not htFechas.ContainsKey(drv.Row("FechaVencimiento")) Then
    '    '            ReDim Preserve FechasCobro(FechasCobro.Length)
    '    '            FechasCobro(FechasCobro.Length - 1) = drv.Row("FechaVencimiento")

    '    '            htFechas(drv.Row("FechaVencimiento")) = drv.Row("FechaVencimiento")
    '    '        End If
    '    '    Next
    '    'End If
    ...
    '    FichCSB19_15_RegistrosCabeceraAcreedorFecha(data, datOrdenante, datBanco, ProcInfo)
    '    FichCSB19_15_RegistrosIndividuales(data, datOrdenante, datBanco, ProcInfo, TotIndividuales)
    '    FichCSB19_15_RegistrosTotalAcreedorFecha(data, datOrdenante, datBanco, ProcInfo, TotAcreedores, TotIndividuales)
    '    ' Next

    '    FichCSB19_15_RegistrosTotalAcreedor(data, datOrdenante, datBanco, ProcInfo, TotGeneral, TotAcreedores)
    'End Sub

    <Task()> Public Shared Sub FichCSB19_15_RegistrosAcreedores(ByVal data As DataAddRegistroFichero, ByVal services As ServiceProvider)
        ...
        For Each FechaCobro As Date In FechasCobro
            ...
            ProcessServer.ExecuteTask(Of DataAddRegistroFichero)(AddressOf FichCSB19_15_RegistrosCabeceraAcreedorFecha, data, services)
            ProcessServer.ExecuteTask(Of DataAddRegistroFichero)(AddressOf FichCSB19_15_RegistrosIndividuales, data, services)
            ProcessServer.ExecuteTask(Of DataAddRegistroFichero)(AddressOf FichCSB19_15_RegistrosTotalAcreedorFecha, data, services)
        Next
        ProcessServer.ExecuteTask(Of DataAddRegistroFichero)(AddressOf FichCSB19_15_RegistrosTotalAcreedor, data, services)
    End Sub

    <Task()> Public Shared Sub FichCSB19_15_RegistrosCabeceraAcreedorFecha(ByVal data As DataAddRegistroFichero, ByVal services As ServiceProvider)
        Dim datBanco As DatosBancoPropioFicheros = services.GetService(Of DatosBancoPropioFicheros)()
        Dim datOrdenante As DatosOrdenante = services.GetService(Of DatosOrdenante)()

        Dim lstRegistro As New ListaCamposRegistro
        lstRegistro.Add(New DataCampoRegistro("Código de Registro", String.Empty, True, enumTipoCampo.Numerico, 2, CodigoRegistroCSB19_15.REG_CABECERA_ACREEDOR_FECHA))
        lstRegistro.Add(New DataCampoRegistro("Versión Cuaderno", String.Empty, True, enumTipoCampo.Numerico, 5, CodigoCuadernoSEPA.CSB19_14_REDUCIDO))
        lstRegistro.Add(New DataCampoRegistro("Número de Dato", String.Empty, True, enumTipoCampo.Numerico, 3, NumeroDatoCSB19_15.CABECERA_ACREEDOR_FECHA))
    ...
    <Task()> Public Shared Sub FichCSB19_15_RegistrosIndividuales(ByVal data As DataAddRegistroFichero, ByVal services As ServiceProvider)
        Dim ProcInfo As ProcessInfoFichero = services.GetService(Of ProcessInfoFichero)()
        If ProcInfo.OrigenDatos Is Nothing OrElse ProcInfo.OrigenDatos.Rows.Count = 0 Then Exit Sub
        ...
        Dim datOrdenante As DatosOrdenante = services.GetService(Of DatosOrdenante)()
        Dim lstRegistro As New ListaCamposRegistro
        lstRegistro.Add(New DataCampoRegistro("Código de Registro", String.Empty, True, enumTipoCampo.Numerico, 2, CodigoRegistroCSB19_15.REG_INDIVIDUAL))
        lstRegistro.Add(New DataCampoRegistro("Versión Cuaderno", String.Empty, True, enumTipoCampo.Numerico, 5, CodigoCuadernoSEPA.CSB19_14_REDUCIDO))
        lstRegistro.Add(New DataCampoRegistro("Número de Dato", String.Empty, True, enumTipoCampo.Numerico, 3, NumeroDatoCSB19_15.INDIVIDUAL_TIPO1))
    ...
    <Task()> Public Shared Sub FichCSB19_15_RegistrosTotalAcreedorFecha(ByVal data As DataAddRegistroFichero, ByVal services As ServiceProvider)
        ...
        Dim lstRegistro As New ListaCamposRegistro
        lstRegistro.Add(New DataCampoRegistro("Código de Registro", String.Empty, True, enumTipoCampo.Numerico, 2, CodigoRegistroCSB19_15.REG_TOTALES_ACREEDOR_FECHA))
        Dim datOrdenante As DatosOrdenante = services.GetService(Of DatosOrdenante)()
    ...
    <Task()> Public Shared Sub FichCSB19_15_RegistrosTotalAcreedor(ByVal data As DataAddRegistroFichero, ByVal services As ServiceProvider)
        Dim TotAcreedores As RegTotalesAcreedores = services.GetService(Of RegTotalesAcreedores)()
        Dim lstRegistro As New ListaCamposRegistro
        lstRegistro.Add(New DataCampoRegistro("Código de Registro", String.Empty, True, enumTipoCampo.Numerico, 2, CodigoRegistroCSB19_15.REG_TOTALES_ACREEDOR))
        Dim datOrdenante As DatosOrdenante = services.GetService(Of DatosOrdenante)()
...
#Region " Métodos Auxiliares para obtener información del sistema "
    ...
    <Task()> Public Shared Sub PrepararInfoGeneralFicheroCORE(ByVal data As DataGenerarFichero, ByVal services As ServiceProvider)
        ...
        Else : ApplicationService.GenerateError("No existen cobros del tipo Mandato COR1.")
        End If
    End Sub
            ...
            Case BusinessEnum.MandatoTipoPago.Recurrente
                ...
                If dtCobrosMandato.Rows.Count > 0 Then
                    If dtCobrosMandato.Rows(0)("IDCobro") = drCobro("IDCobro") Then
                        TipoAdeudo = TipoAdeudoCSB19_15.ADEUDO_RECURRENTE_PRIMERO
                        'ElseIf dtCobrosMandato.Rows(dtCobrosMandato.Rows.Count - 1)("IDCobro") = drCobro("IDCobro") Then
                        '    TipoAdeudo = TipoAdeudoCSB19_15.ADEUDO_RECURRENTE_ULTIMO
                    Else
                        TipoAdeudo = TipoAdeudoCSB19_15.ADEUDO_RECURRENTE_INTERMEDIO
                    End If
                End If
        End Select
        Return TipoAdeudo
