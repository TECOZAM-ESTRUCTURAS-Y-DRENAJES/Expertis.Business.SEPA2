...
#Region " Constantes Modelo CSB19_44 "

Public Module CodigoRegistroCSB19_44
...
Public Module NumeroDatoCSB19_44
...
Public Module TipoAdeudoCSB19_44
...
Public Module PrefijoFichAdeudoCSB19_44
...
Public Class FicheroCSB19_44_B2B
    ...
    <Task()> Public Shared Function GenerarFichero(ByVal data As DataGenerarFichero, ByVal services As ServiceProvider) As List(Of String)
        ...
        ProcessServer.ExecuteTask(Of DataGenerarFichero)(AddressOf PrepararInfoGeneralFicheroB2B, data, services)

        Dim datFichero As New DataAddRegistroFichero
        ProcessServer.ExecuteTask(Of DataAddRegistroFichero)(AddressOf FichCSB19_44_RegistroCabeceraGeneral, datFichero, services)
        ProcessServer.ExecuteTask(Of DataAddRegistroFichero)(AddressOf FichCSB19_44_RegistrosAcreedores, datFichero, services)
        ProcessServer.ExecuteTask(Of DataAddRegistroFichero)(AddressOf FichCSB19_44_RegistroTotalesGeneral, datFichero, services)
...
#Region " Identificación del Fichero "
    ...
    <Task()> Public Shared Function GetIdentificacionFichero(ByVal data As Object, ByVal services As ServiceProvider) As String
        ...
        Return PrefijoFichAdeudoCSB19_44.FICHERO_PRESEN_ADEUDOS & Format(Today, FormatosFechas.AAAAMMDD) & Format(Date.Now, FormatosFechas.HHmmssfffff) & datOrdenante.NIF
    End Function
...
#Region " Cabecera y Totales "
    ...
    <Task()> Public Shared Sub FichCSB19_44_RegistroCabeceraGeneral(ByVal data As DataAddRegistroFichero, ByVal services As ServiceProvider)
        ...
        lstRegistro.Add(New DataCampoRegistro("Versión Cuaderno", String.Empty, True, enumTipoCampo.Numerico, 5, CodigoCuadernoSEPA.CSB19_44))
        lstRegistro.Add(New DataCampoRegistro("Número de Dato", String.Empty, True, enumTipoCampo.Numerico, 3, NumeroDatoCSB19_44.CABECERA))
    ...
    <Task()> Public Shared Sub FichCSB19_44_RegistroTotalesGeneral(ByVal data As DataAddRegistroFichero, ByVal services As ServiceProvider)
        Dim TotGeneral As RegTotalesGeneral = services.GetService(Of RegTotalesGeneral)()

        Dim lstRegistro As New ListaCamposRegistro
        lstRegistro.Add(New DataCampoRegistro("Código de Registro", String.Empty, True, enumTipoCampo.Numerico, 2, CodigoRegistroCSB19_44.REG_TOTALES_GENERAL))
...
#Region " Registros de Acreedores - Cabeceras, Totales e Individuales "
    ...
    <Task()> Public Shared Sub FichCSB19_44_RegistrosAcreedores(ByVal data As DataAddRegistroFichero, ByVal services As ServiceProvider)
        ...
        For Each FechaCobro As Date In FechasCobro
            ...
            ProcessServer.ExecuteTask(Of DataAddRegistroFichero)(AddressOf FichCSB19_44_RegistrosCabeceraAcreedorFecha, data, services)
            ProcessServer.ExecuteTask(Of DataAddRegistroFichero)(AddressOf FichCSB19_44_RegistrosIndividuales, data, services)
            ProcessServer.ExecuteTask(Of DataAddRegistroFichero)(AddressOf FichCSB19_44_RegistrosTotalAcreedorFecha, data, services)
        Next

        ProcessServer.ExecuteTask(Of DataAddRegistroFichero)(AddressOf FichCSB19_44_RegistrosTotalAcreedor, data, services)
    End Sub

    <Task()> Public Shared Sub FichCSB19_44_RegistrosCabeceraAcreedorFecha(ByVal data As DataAddRegistroFichero, ByVal services As ServiceProvider)
        Dim datBanco As DatosBancoPropioFicheros = services.GetService(Of DatosBancoPropioFicheros)()
        Dim datOrdenante As DatosOrdenante = services.GetService(Of DatosOrdenante)()

        Dim lstRegistro As New ListaCamposRegistro
        lstRegistro.Add(New DataCampoRegistro("Código de Registro", String.Empty, True, enumTipoCampo.Numerico, 2, CodigoRegistroCSB19_44.REG_CABECERA_ACREEDOR_FECHA))
        lstRegistro.Add(New DataCampoRegistro("Versión Cuaderno", String.Empty, True, enumTipoCampo.Numerico, 5, CodigoCuadernoSEPA.CSB19_44))
        lstRegistro.Add(New DataCampoRegistro("Número de Dato", String.Empty, True, enumTipoCampo.Numerico, 3, NumeroDatoCSB19_44.CABECERA_ACREEDOR_FECHA))
    ...
    <Task()> Public Shared Sub FichCSB19_44_RegistrosIndividuales(ByVal data As DataAddRegistroFichero, ByVal services As ServiceProvider)
        Dim ProcInfo As ProcessInfoFichero = services.GetService(Of ProcessInfoFichero)()
        If ProcInfo.OrigenDatos Is Nothing OrElse ProcInfo.OrigenDatos.Rows.Count = 0 Then Exit Sub
        ...
        Dim datOrdenante As DatosOrdenante = services.GetService(Of DatosOrdenante)()
        Dim lstRegistro As New ListaCamposRegistro
        lstRegistro.Add(New DataCampoRegistro("Código de Registro", String.Empty, True, enumTipoCampo.Numerico, 2, CodigoRegistroCSB19_44.REG_INDIVIDUAL))
        lstRegistro.Add(New DataCampoRegistro("Versión Cuaderno", String.Empty, True, enumTipoCampo.Numerico, 5, CodigoCuadernoSEPA.CSB19_44))
        lstRegistro.Add(New DataCampoRegistro("Número de Dato", String.Empty, True, enumTipoCampo.Numerico, 3, NumeroDatoCSB19_44.INDIVIDUAL_TIPO1))
    ...
    <Task()> Public Shared Sub FichCSB19_44_RegistrosTotalAcreedorFecha(ByVal data As DataAddRegistroFichero, ByVal services As ServiceProvider)
        ...
        Dim lstRegistro As New ListaCamposRegistro
        lstRegistro.Add(New DataCampoRegistro("Código de Registro", String.Empty, True, enumTipoCampo.Numerico, 2, CodigoRegistroCSB19_44.REG_TOTALES_ACREEDOR_FECHA))
        Dim datOrdenante As DatosOrdenante = services.GetService(Of DatosOrdenante)()
    ...
    <Task()> Public Shared Sub FichCSB19_44_RegistrosTotalAcreedor(ByVal data As DataAddRegistroFichero, ByVal services As ServiceProvider)
        Dim TotAcreedores As RegTotalesAcreedores = services.GetService(Of RegTotalesAcreedores)()
        Dim lstRegistro As New ListaCamposRegistro
        lstRegistro.Add(New DataCampoRegistro("Código de Registro", String.Empty, True, enumTipoCampo.Numerico, 2, CodigoRegistroCSB19_44.REG_TOTALES_ACREEDOR))
        Dim datOrdenante As DatosOrdenante = services.GetService(Of DatosOrdenante)()
...
#Region " Métodos Auxiliares para obtener información del sistema "
    ...
    <Task()> Public Shared Sub PrepararInfoGeneralFicheroB2B(ByVal data As DataGenerarFichero, ByVal services As ServiceProvider)
        ...
        Dim dtCobros As DataTable = New BE.DataEngine().Filter("vSEPA_RemesaDefinitivaGenFichero_19_44", f)
        ...
        Else : ApplicationService.GenerateError("No existen cobros del tipo Mandato B2B.")
        End If
    End Sub
    ...
    <Task()> Public Shared Function GetTipoAdeudo(ByVal drCobro As DataRow, ByVal services As ServiceProvider) As String
        ...
        Select Case drCobro("TipoPago")
            Case BusinessEnum.MandatoTipoPago.Unico
                TipoAdeudo = TipoAdeudoCSB19_44.ADEUDO_UNICO
            Case BusinessEnum.MandatoTipoPago.Recurrente
                ...
                If dtCobrosMandato.Rows.Count > 0 Then
                    If dtCobrosMandato.Rows(0)("IDCobro") = drCobro("IDCobro") Then
                        TipoAdeudo = TipoAdeudoCSB19_44.ADEUDO_RECURRENTE_PRIMERO
                        'ElseIf dtCobrosMandato.Rows(dtCobrosMandato.Rows.Count - 1)("IDCobro") = drCobro("IDCobro") Then
                        '    TipoAdeudo = TipoAdeudoCSB19_44.ADEUDO_RECURRENTE_ULTIMO
                    Else
                        TipoAdeudo = TipoAdeudoCSB19_44.ADEUDO_RECURRENTE_INTERMEDIO
                    End If
                End If
        End Select
        Return TipoAdeudo
    ...
    '    'Dim dtCobro As DataTable = New BE.DataEngine().Filter("ViewNFacturaGenerarFicheros", New NumberFilterItem("IDCobroAgrupado", IDCobro), "NFactura")
